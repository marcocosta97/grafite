...
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test corr $DATASETS_PATH/corr_test $GRAFITE_PATH ; then
  echo "[!!] corr_test test failed"
  exit 1
fi
echo "[!!] corr_test (figure 1, 5) test executed successfully"
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test lemma $DATASETS_PATH/lemma_test $GRAFITE_PATH ; then
  echo "[!!] lemma_test test failed"
  exit 1
fi
echo "[!!] lemma_test (figure 2) test executed successfully"
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test fpr $DATASETS_PATH/fpr_test $GRAFITE_PATH ; then
  echo "[!!] fpr_test test failed"
  exit 1
fi
echo "[!!] fpr_test (figure 3) test executed successfully"
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test fpr_real $DATASETS_PATH/fpr_real_test $GRAFITE_PATH ; then
  echo "[!!] fpr_real_test test failed"
  exit 1
fi
echo "[!!] fpr_real_test (figure 4) test executed successfully"
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test true $DATASETS_PATH/true_test $GRAFITE_PATH ; then
  echo "[!!] query_time_test test failed"
  exit 1
fi
echo "[!!] query_time_test (figure 6) test executed successfully"
if ! python3 $SCRIPT_DIR_PATH/test.py $ARGS --test constr_time $DATASETS_PATH/constr_time_test $GRAFITE_PATH ; then
